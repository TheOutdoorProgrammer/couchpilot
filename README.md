...
- **🔍 Full code review** — opt a session into review mode and every file change Claude attempts is held for your approval first: a syntax-highlighted diff on your phone, expandable up/down for more of the surrounding file, comments on a single line or a multi-line range (editable after the fact), overall comments, and **Approve** / **Request changes**. Denials feed your comments straight back to Claude, which revises and resubmits.
...
- **Comment on a line or a range.** Tap a line to start a selection, then tap the neighbouring line to grow it (or an endpoint to shrink) — the span highlights and the comment is tagged *lines A–B*. The speech-bubble button adds an overall comment. Every comment carries a **✎** to edit its text and **✕** to delete; tapping a line inside a saved comment re-opens its range to resize.
...
- **Deny** (requires at least one comment — Claude needs something to act on) blocks the write and hands Claude your comments, each quoted against the line or range it targets. Claude revises and the next attempt comes back as a fresh review.
